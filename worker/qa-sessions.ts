...
const PERSONAS = {
  ...
  player: { identityId: "identity-combat-qa-player", membershipId: "membership-combat-qa-player", name: "QA Player", role: "player" as const },
};
...
export async function handleQaSession(
  ...
  if (!fixture) return json({ error: "The combat rolling QA fixture is unavailable." }, { status: 503 });
  const now = Date.now();
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(
      `UPDATE tokens SET hp = CASE id
         ...
         WHEN 'token-combat-qa-skeleton' THEN 13 WHEN 'token-combat-qa-unconfigured' THEN 7 ELSE hp END,
       x = CASE id
         WHEN 'token-combat-qa-player' THEN 4 WHEN 'token-combat-qa-goblin' THEN 9
         WHEN 'token-combat-qa-skeleton' THEN 10 WHEN 'token-combat-qa-unconfigured' THEN 9 ELSE x END,
       y = CASE id
         WHEN 'token-combat-qa-player' THEN 5.5 WHEN 'token-combat-qa-goblin' THEN 3.5
         WHEN 'token-combat-qa-skeleton' THEN 5.5 WHEN 'token-combat-qa-unconfigured' THEN 7.5 ELSE y END,
