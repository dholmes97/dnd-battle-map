...
test("includes a durable full-scene workshop with no retired editor path", async () => {
  ...
  assert.match(workshopSource, /className="workshop-header-tools"/);
  ...
  assert.match(workshopSource, /const wantsUndo = modifier && key === "z" && !event\.shiftKey/);
  assert.match(workshopSource, /target\?\.isContentEditable \|\| target\?\.closest\("input, textarea, select"\)/);
  assert.doesNotMatch(workshopSource, /draft-history-row/);
  ...
  assert.doesNotMatch(workshopSource, /mapPackage: \{ \.\.\.map, name \}/);
  ...
  assert.match(styles, /\.full-scene-list \{[^}]*max-height: 15rem;[^}]*overflow-y: auto/);
