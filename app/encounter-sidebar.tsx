...
import type { TokenPreview } from "@/app/battle-map-renderer";
...
export function EncounterSidebar({ participant, state, hidden, inCombat, rosterFilter, rosterRows, selectedToken, selectedSpell, selectedMapNote, preview, distance, remainingMovement, overMovement, activeOwnTurnToken, activeOwnTurnIsGroup, initiativeTokens, encounterAction, controls, onRosterFilterChange, onToggleGroup, onSelectToken, onCloseMapNote, onResizeSpell, onDeleteToken, canMoveToken, onHideToken, onEndTurn, onStartOrRestart, onAdvanceTurn, onPauseOrResume, onRequestReset, onCorrectTurn }: EncounterSidebarProps) {
  ...
  const hpStep = Math.max(1, Math.trunc(Number(controls.hpAmount)) || 1);
  ...
  const rosterRow = (token: SharedToken, grouped: boolean) => {
    ...
    return <button key={token.id} type="button" className={`roster-row${selected ? " is-selected" : ""}${active ? " is-active" : ""}${token.turnComplete ? " is-complete" : ""}${grouped ? " is-grouped" : ""}${token.controlledByViewer ? " is-mine" : ""}`} aria-pressed={selected} disabled={isPendingCreate(token)} onClick={() => onSelectToken(token.id)}>
  ...
  return <aside className="control-panel" aria-label="Encounter controls" hidden={hidden}>
    ...
    <div className="token-roster" role="list" aria-label={inCombat ? "Turn order" : "Tokens"}>
      ...
      {rosterRows.map((row) => row.type === "group" ? <div className={`roster-group${row.tokens[0].initiativeOrder !== null && row.tokens[0].initiativeOrder === state.encounter.activeInitiativeOrder ? " is-active" : ""}`} key={row.key}>
        <button type="button" className="roster-row is-group" aria-expanded={row.expanded} onClick={() => onToggleGroup(row.key)}>
          ...
          <span className="roster-name">{row.label}<em> ×{row.tokens.length}</em></span><span className="roster-group-toggle">{row.expanded ? "Hide" : "Show"}</span><span className="roster-initiative">{row.tokens[0].initiative ?? "—"}</span>
        </button>
        {!inCombat && participant.role === "dm" ? <div className="roster-group-initiative"><label htmlFor={`initiative-${row.key}`}>All</label><input id={`initiative-${row.key}`} aria-label={`Initiative for all ${row.label} creatures`} type="text" inputMode="numeric" pattern="[0-9]*" maxLength={2} placeholder={row.tokens.every((token) => token.initiative === row.tokens[0].initiative) && row.tokens[0].initiative !== null ? String(row.tokens[0].initiative) : "—"} value={controls.initiativeDrafts[`group:${row.key}`] ?? ""} onChange={(event) => { const next = event.target.value.replace(/\D/g, "").slice(0, 2); controls.setInitiativeDrafts((current) => ({ ...current, [`group:${row.key}`]: next })); controls.setInitiativeStatuses((current) => ({ ...current, [`group:${row.key}`]: "editing" })); }} onBlur={() => void controls.saveInitiativeGroup(row.key, row.tokens)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} /><span aria-live="polite">{controls.initiativeStatuses[`group:${row.key}`] === "saving" ? "Saving…" : "initiative"}</span></div> : null}
      </div> : rosterRow(row.token, row.grouped))}
    </div>
    {selectedMapNote ? <section className="map-note-detail" aria-label={`DM note ${state.encounter.mapPackage!.notes.findIndex((note) => note.id === selectedMapNote.id) + 1}`}><div className="map-note-heading"><div><small>Private map note</small><h2>Note {state.encounter.mapPackage!.notes.findIndex((note) => note.id === selectedMapNote.id) + 1}</h2></div><IconActionButton variant="close" label="Close DM note" onClick={onCloseMapNote} /></div><p>{selectedMapNote.text}</p></section>
      ...
      : selectedToken ? <TokenDetails participant={participant} state={state} token={selectedToken} health={selectedHealth} preview={preview} distance={distance} remainingMovement={remainingMovement} overMovement={overMovement} hpStep={hpStep} controls={controls} onHideToken={onHideToken} onDeleteToken={onDeleteToken} /> : null}
...
function TokenDetails({ participant, state, token, health, preview, distance, remainingMovement, overMovement, hpStep, controls, onHideToken, onDeleteToken }: { participant: ParticipantSession; state: EncounterState; token: SharedToken; health: ReturnType<typeof displayHealth>; preview: TokenPreview | null; distance: number; remainingMovement: number; overMovement: boolean; hpStep: number; controls: TokenControls; onHideToken: (token: SharedToken) => void; onDeleteToken: (token: SharedToken) => void }) {
  ...
  return <section className="token-detail" aria-label={`${token.name} details`}>
    ...
    {controlled && token.hp !== null && token.maxHp !== null ? <div className="hp-panel"><div className="hp-readout"><strong>HP {token.hp}/{token.maxHp}</strong><span className={`hp-track is-${health?.band ?? "unharmed"}`}><span className="hp-track-fill" style={{ width: `${Math.round((health?.ratio ?? 0) * 100)}%`, background: health?.color }} /></span></div><div className="hp-row"><button className="hp-step" aria-label="Decrease amount" onClick={() => controls.setHpAmount(String(Math.max(1, hpStep - 1)))}>−</button><input aria-label="HP change amount" type="text" inputMode="numeric" pattern="[0-9]*" value={controls.hpAmount} onChange={(event) => controls.setHpAmount(event.target.value.replace(/\D/g, "").slice(0, 3))} onKeyDown={(event) => { if (event.key === "Enter") void controls.applyHpToToken(token, -hpStep); }} /><button className="hp-step" aria-label="Increase amount" onClick={() => controls.setHpAmount(String(hpStep + 1))}>+</button><button className="hp-apply is-damage" onClick={() => void controls.applyHpToToken(token, -hpStep)}>Damage</button><button className="hp-apply is-heal" onClick={() => void controls.applyHpToToken(token, hpStep)}>Heal</button></div></div> : null}
