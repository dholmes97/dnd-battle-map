...
type EncounterSidebarProps = {
  ...
  onBeginAttack: (token: SharedToken) => void;
  onCloseMapNote: () => void;
...
export function EncounterSidebar({ participant, state, hidden, inCombat, rosterRows, selectedToken, selectedSpell, selectedMapNote, activeOwnTurnToken, activeOwnTurnIsGroup, initiativeTokens, encounterAction, controls, onToggleGroup, onSelectToken, onBeginAttack, onCloseMapNote, onResizeSpell, onDeleteToken, canMoveToken, onHideToken, onEndTurn, onStartOrRestart, onAdvanceTurn, onPauseOrResume, onRequestReset, onCorrectTurn }: EncounterSidebarProps) {
  ...
  const playerCharacter = participant.role === "player"
    ...
    : null;
  const [pinnedTokenId, setPinnedTokenId] = useState<string | null>(() => playerCharacter?.id ?? null);
  const pinnedToken = state.tokens.find((token) => token.id === pinnedTokenId) ?? null;
  ...
  const tokenDetails = (token: SharedToken) => <TokenDetails participant={participant} state={state} token={token} health={displayHealth(token.hp, token.maxHp, token.healthState)} hpStep={hpStep} controls={controls} onBeginAttack={onBeginAttack} onHideToken={onHideToken} onDeleteToken={onDeleteToken} />;
  const tokenCard = (token: SharedToken, spell: SpellEffectDefinition | null) => spell
    ? <section className={`spell-detail is-${spell.id}`} aria-label={`${token.name} spell effect details`}><div className={`spell-detail-visual${spell.shape ? " is-generic-shape" : ""}`}>{spell.shape ? <SpellShapeMark shape={spell.shape} /> : <NextImage src={spell.artAsset} alt="" width={180} height={180} unoptimized />}</div><div className="spell-detail-copy"><small>Persistent spell · controlled by {token.controller.name}</small><h2>{token.name}</h2><p>{spell.description}</p></div><div className="spell-detail-meta"><span><small>Area</small><strong>{spellFootprintLabel(spell, token.size)}</strong></span><span><small>Movement</small><strong>{canMoveToken(token) ? "Drag directly" : `Owner only · ${token.controller.name}`}</strong></span></div>{token.controlledByViewer ? <label className="spell-size-control">Footprint<select aria-label="Spell footprint size" value={SPELL_AREA_SIZES.includes(token.size as SpellAreaSize) ? token.size : "medium"} onChange={(event) => onResizeSpell(token, spell, event.target.value as SpellAreaSize)}>{SPELL_AREA_SIZES.map((size) => <option value={size} key={size}>{spell.shape === "square" ? `${spellAreaDiameter(size)}-ft square` : `${spellAreaDiameter(size)}-ft diameter`}</option>)}</select></label> : null}{token.controlledByViewer ? <button className="dismiss-spell-button" onClick={() => onDeleteToken(token)}>Dismiss {spell.name}</button> : null}</section>
    : tokenDetails(token);
  const dockableTokenCard = (token: SharedToken, spell: SpellEffectDefinition | null, pinned: boolean) => <div className={`dockable-card${pinned ? " is-pinned" : ""}`}>
    <button type="button" className="card-lock-button" aria-label={`${pinned ? "Unlock" : "Lock"} ${token.name} card`} aria-pressed={pinned} data-tooltip={pinned ? "Unlock card from the bottom of the sidebar" : "Lock card at the bottom of the sidebar"} onClick={() => setPinnedTokenId(pinned ? null : token.id)}><Icon name={pinned ? "lock" : "unlock"} /></button>
    {tokenCard(token, spell)}
  </div>;
  const selectedCard = selectedMapNote ? <section className="map-note-detail" aria-label={`DM note ${state.encounter.mapPackage!.notes.findIndex((note) => note.id === selectedMapNote.id) + 1}`}><div className="map-note-heading"><div><small>Private map note</small><h2>Note {state.encounter.mapPackage!.notes.findIndex((note) => note.id === selectedMapNote.id) + 1}</h2></div><IconActionButton variant="close" label="Close DM note" onClick={onCloseMapNote} /></div><p>{selectedMapNote.text}</p></section>
    : selectedToken ? dockableTokenCard(selectedToken, selectedSpell, selectedToken.id === pinnedToken?.id) : null;
  const selectionCard = selectedToken?.id === pinnedToken?.id ? null : selectedCard;
  ...
  return <aside className={`control-panel is-${participant.role}`} aria-label="Encounter controls" hidden={hidden}>
    ...
    {pinnedCard ? <div className={`sidebar-card-stack${participant.role === "dm" ? " dm-card-stack" : ""}${selectionCard ? " has-selection" : ""}`} role="group" aria-label="Locked card and selection">
      {selectionCard ? <div className="sidebar-card-layer is-selection">{selectionCard}</div> : null}
...
function TokenDetails({ participant, state, token, health, hpStep, controls, onBeginAttack, onHideToken, onDeleteToken }: { participant: ParticipantSession; state: EncounterState; token: SharedToken; health: ReturnType<typeof displayHealth>; hpStep: number | null; controls: TokenControls; onBeginAttack: (token: SharedToken) => void; onHideToken: (token: SharedToken) => void; onDeleteToken: (token: SharedToken) => void }) {
  ...
  return <section className="token-detail" aria-label={`${token.name} details`}>
    ...
    {controlled && token.hp !== null && token.maxHp !== null ? <div className="hp-panel"><div className="hp-readout"><strong>HP {token.hp}/{token.maxHp}{token.temporaryHp ? ` + ${token.temporaryHp} temp` : ""}</strong><span className={`hp-track is-${health?.band ?? "unharmed"}`}><span className="hp-track-fill" style={{ width: `${Math.round((health?.ratio ?? 0) * 100)}%`, background: health?.color }} /></span></div><div className="hp-row"><input aria-label="HP change amount" aria-invalid={controls.hpAmount !== "" && hpStep === null} title="HP amount" placeholder="HP" type="text" inputMode="numeric" pattern="[0-9]*" value={controls.hpAmount} onFocus={(event) => event.currentTarget.select()} onClick={(event) => event.currentTarget.select()} onChange={(event) => controls.setHpAmount(event.target.value.replace(/\D/g, "").slice(0, 3))} onKeyDown={(event) => { if (event.key === "Enter" && hpStep !== null) void controls.applyHpToToken(token, -hpStep); }} /><button className="hp-apply is-damage" aria-label={hpStep === null ? `Enter an HP amount before damaging ${token.name}` : `Apply ${hpStep} damage to ${token.name}`} data-tooltip={hpStep === null ? "Enter an HP amount" : `Damage ${hpStep} HP`} disabled={hpStep === null} onClick={() => { if (hpStep !== null) void controls.applyHpToToken(token, -hpStep); }}><Icon name="damage" /></button><button className="hp-apply is-heal" aria-label={hpStep === null ? `Enter an HP amount before healing ${token.name}` : `Heal ${token.name} for ${hpStep}`} data-tooltip={hpStep === null ? "Enter an HP amount" : `Heal ${hpStep} HP`} disabled={hpStep === null} onClick={() => { if (hpStep !== null) void controls.applyHpToToken(token, hpStep); }}><Icon name="heal" /></button><label className="temp-hp-control"><span>Temp HP</span><input aria-label={`${token.name} temporary HP`} type="text" inputMode="numeric" pattern="[0-9]*" value={controls.temporaryHpDrafts[token.id] ?? token.temporaryHp ?? 0} onChange={(event) => controls.setTemporaryHpDrafts((current) => ({ ...current, [token.id]: event.target.value.replace(/\D/g, "").slice(0, 6) }))} onBlur={() => void controls.saveTemporaryHp(token)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); if (event.key === "Escape") { controls.setTemporaryHpDrafts((current) => { const next = { ...current }; delete next[token.id]; return next; }); event.currentTarget.blur(); } }} /></label></div></div> : null}
