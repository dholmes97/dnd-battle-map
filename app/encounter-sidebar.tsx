...
function TokenDetails({ participant, state, token, health, preview, distance, remainingMovement, overMovement, hpStep, controls, onHideToken, onDeleteToken }: { participant: ParticipantSession; state: EncounterState; token: SharedToken; health: ReturnType<typeof displayHealth>; preview: TokenPreview | null; distance: number; remainingMovement: number; overMovement: boolean; hpStep: number; controls: TokenControls; onHideToken: (token: SharedToken) => void; onDeleteToken: (token: SharedToken) => void }) {
  ...
  return <section className="token-detail" aria-label={`${token.name} details`}>
    ...
    {controlled && token.hp !== null && token.maxHp !== null ? <div className="hp-panel"><div className="hp-readout"><strong>HP {token.hp}/{token.maxHp}</strong><span className={`hp-track is-${health?.band ?? "unharmed"}`}><span className="hp-track-fill" style={{ width: `${Math.round((health?.ratio ?? 0) * 100)}%`, background: health?.color }} /></span></div><div className="hp-row"><input aria-label="HP change amount" title="HP amount" type="text" inputMode="numeric" pattern="[0-9]*" value={controls.hpAmount} onFocus={(event) => event.currentTarget.select()} onClick={(event) => event.currentTarget.select()} onChange={(event) => controls.setHpAmount(event.target.value.replace(/\D/g, "").slice(0, 3))} onKeyDown={(event) => { if (event.key === "Enter") void controls.applyHpToToken(token, -hpStep); }} /><button className="hp-apply is-damage" aria-label={`Apply ${hpStep} damage to ${token.name}`} data-tooltip={`Damage ${hpStep} HP`} onClick={() => void controls.applyHpToToken(token, -hpStep)}><Icon name="damage" /></button><button className="hp-apply is-heal" aria-label={`Heal ${token.name} for ${hpStep}`} data-tooltip={`Heal ${hpStep} HP`} onClick={() => void controls.applyHpToToken(token, hpStep)}><Icon name="heal" /></button></div></div> : null}
