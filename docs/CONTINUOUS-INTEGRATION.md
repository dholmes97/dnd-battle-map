...
Run the full pre-publish gate locally with `npm run verify` and `npm run audit:dependencies`. `npm run verify` includes both enforced coverage suites; `npm run test:coverage` remains available when only the coverage gate needs to be checked. Install the browser runtimes once with `npx playwright install chromium webkit` when they are not already present.
