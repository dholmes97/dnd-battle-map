...
- Apply that optimistic rule to every battle-map interaction: annotations, initiative, HP, effects, token details/visibility, combat controls, encounter state, and local undo/redo must render immediately for the initiating participant while pending reducers survive live refreshes; reconcile to authoritative state and roll back only rejected mutations.
